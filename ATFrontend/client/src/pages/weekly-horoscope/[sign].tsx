...
export default function WeeklyHoroscopeSign() {
  ...
  const { data: weeklyHoroscopes, isLoading } = useQuery<{
    ...
    startDate: string;
    endDate: string;
    horoscopes: WeeklyHoroscopeData[];
  }>({
  ...
  const currentHoroscope = weeklyHoroscopes?.horoscopes?.find(
    (h: WeeklyHoroscopeData) => h.sign === finalSign,
  );
  ...
  return (
    <>
      <Helmet>
        <title>{`${finalSign} Weekly Horoscope - ${weeklyHoroscopes?.startDate && weeklyHoroscopes?.endDate ? formatDateRange(weeklyHoroscopes.startDate, weeklyHoroscopes.endDate) : "This Week"} | Vedic Astrology`}</title>
        <meta
          name="description"
          content={`Get your ${finalSign} weekly horoscope predictions. Authentic Vedic astrology insights for love, career, health, and finances. Sanskrit name: ${vedicData?.sanskrit || finalSign}.`}
        />
        <meta
          name="keywords"
          content={`${finalSign} weekly horoscope, ${finalSign} week predictions, ${vedicData?.sanskrit}, ${finalSign} astrology, weekly horoscope ${finalSign.toLowerCase()}`}
        />
        <link
          rel="canonical"
          href={`/weekly-horoscope/${finalSign.toLowerCase()}`}
        />
      </Helmet>
            ...
            <div className="flex justify-center items-center space-x-4">
              <div className="w-16 h-16 rounded-full bg-white dark:bg-gray-800 shadow-lg flex items-center justify-center">
                <img
                  ...
                  alt={finalSign}
                  className="w-12 h-12 md:w-16 md:h-16 object-cover transition-transform duration-300 hover:scale-110 border border-gray-300 dark:border-gray-600 rounded-full"
                  onError={(e) => {
              ...
              <div>
                <h1 className="text-2xl md:text-5xl font-bold bg-gradient-to-r from-purple-600 to-blue-600 dark:from-purple-400 dark:to-blue-400 bg-clip-text text-transparent">
                  {finalSign} Weekly Horoscope
                </h1>
                <p className="text-lg text-gray-600 dark:text-gray-300">
                  {vedicData?.sanskrit} ({vedicData?.hindi}) -{" "}
                  {weeklyHoroscopes &&
                    formatDateRange(
                      weeklyHoroscopes.startDate,
                      weeklyHoroscopes.endDate,
                    )}
                </p>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg">
            ...
            <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-12 gap-3">
              {zodiacSigns.map((sign) => (
                <Link
                  key={sign.name}
                  href={`/weekly-horoscope/${sign.name.toLowerCase()}`}
                >
                  <div
                  ...
                  >
                    <img
                      ...
                      alt={sign.name}
                      className="w-8 h-8 md:w-16 md:h-16  md:object-cover mx-auto mb-1 border border-gray-300 dark:border-gray-600 rounded-full"
                      onError={(e) => {
                        const target = e.target as HTMLImageElement;
                        target.style.display = "none";
                        if (target.parentElement) {
            ...
            <div className="space-y-6">
              ...
              <Card className="bg-white dark:bg-gray-800 shadow-xl border-0">
                <CardHeader className="text-center">
                  <div className="flex justify-center items-center space-x-2 mb-2">
                    ...
                    <span className="text-sm text-gray-500 dark:text-gray-400">
                      Week of {formatDateRange(weeklyHoroscopes.startDate)}
                    </span>
                  </div>
                  <CardTitle className="text-2xl font-bold text-gray-800 dark:text-gray-200">
                    Weekly Forecast for {finalSign}
                  </CardTitle>
                ...
                <CardContent className="space-y-6">
                  ...
                  {/* Key Themes */}
                  {currentHoroscope.keyThemes &&
                    currentHoroscope.keyThemes.length > 0 && (
                      <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-lg">
                        <h4 className="font-semibold text-indigo-800 dark:text-indigo-400 mb-3">
                          Key Themes This Week
                        </h4>
                        <div className="flex flex-wrap gap-2">
                          {currentHoroscope.keyThemes.map((theme, index) => (
                            <Badge
                              key={index}
                              variant="secondary"
            ...
            <Card className="bg-white dark:bg-gray-800 shadow-xl">
              <CardContent className="p-8 text-center">
                <p className="text-gray-600 dark:text-gray-400">
                  No weekly horoscope data available.
                </p>
              </CardContent>
            </Card>
          )}
